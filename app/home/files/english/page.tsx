...
export default function EnglishFilesPage() {
  ...
  const fetchPdfs = async (token: string) => {
    ...
    try {
      const response = await fetch(buildApiUrlNoPort(`${API_ENDPOINTS.PDF_LIST}?subject=english`), {
        headers: {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.append("subject", "english") // Hardcoded for this page
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          ...
          console.error("Error parsing uploaded PDF data:", e)
          // Refresh the file list instead
          if (authToken) {
            fetchPdfs(authToken)
          }
        }
        ...
        toast({
          title: "Upload Failed",
          description: result.message || "An error occurred while uploading the PDF.",
          variant: "destructive"
        })
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : "Unknown error uploading PDF"
      toast({
        title: "Error",
        description: errorMessage,
        variant: "destructive"
  ...
  const handleDeleteFile = async (id: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "File Deleted",
          description: result.message || "The file has been removed from your collection.",
          variant: "success",
        })
      } else {
        toast({
          title: "Delete Failed",
          description: result.message || "An error occurred while deleting the file.",
          variant: "destructive",
        })
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : "Unknown error deleting PDF"
      toast({
        title: "Error",
        description: errorMessage,
        variant: "destructive"
  ...
  const handleAssessFile = async (id: string) => {
    ...
    if (selectedFile.status !== "ready") {
      toast({
        ...
        description: "This file is still processing or has encountered an error. Please wait or try again.",
        variant: "destructive",
      })
      return
    }
  ...
  const handleLearnFromFile = async (id: string, question: string) => {
    if (!authToken) {
      toast({
        title: "Authentication Error",
        description: "You must be logged in to use the learning feature.",
        variant: "destructive"
      })
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        // Navigate to the learning page with the response data
        // This would typically be handled by a router
        window.location.href = `/home/learning/english/${id}?question=${encodeURIComponent(question)}`
      } else {
        toast({
          title: "Learning Request Failed",
          description: result.message || "An error occurred while processing your learning request.",
          variant: "destructive",
        })
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : "Unknown error processing learning request"
      toast({
        title: "Error",
        description: errorMessage,
        variant: "destructive"
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: result.Message || "An error occurred while generating the assessment.",
          variant: "destructive",
        })
        return null
      }
  ...
  // Add handler for opening the learning modal
  ...
  const handleLearningSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedPdfId || !learningQuestion.trim()) {
      toast({
        title: "Input Required",
        description: "Please enter a question to learn from the PDF.",
        variant: "destructive",
      })
      return
    }
    ...
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="text-center">
          ...
          <p className="text-gray-500">Loading your English files...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="flex items-center mb-4 md:mb-6">
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-brand-navy">English Files</h1>
      </div>

      <p className="text-sm md:text-base text-gray-600 mb-6 md:mb-8">
        Upload and manage your English files, notes, and resources.
      </p>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="files">
          <Card>
            <CardHeader className="p-4 md:p-6">
              <CardTitle className="text-lg md:text-xl">English Files</CardTitle>
              <CardDescription className="text-sm">Manage your uploaded English files</CardDescription>
            </CardHeader>
              ...
              {files.length > 0 ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-500 mb-6">
                    You haven't uploaded any English files yet.
                  </p>
        ...
        <TabsContent value="upload">
          <Card>
            <CardHeader className="p-4 md:p-6">
              <CardTitle className="text-lg md:text-xl">Upload English File</CardTitle>
              <CardDescription className="text-sm">
                Upload PDF files for English subject
              </CardDescription>
            </CardHeader>
            <CardContent className="p-4 md:p-6 pt-0 md:pt-0">
