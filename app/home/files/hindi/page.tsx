...
export default function HindiFilesPage() {
  ...
  const fetchPdfs = async (token: string) => {
    ...
    try {
      const response = await fetch(buildApiUrlNoPort(`${API_ENDPOINTS.PDF_LIST}?subject=hindi`), {
        headers: {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.append("subject", "hindi") // Hardcoded for this page
  ...
  const handleLearnFromFile = async (id: string, question: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        window.location.href = `/home/learning/hindi/${id}?question=${encodeURIComponent(question)}`
      } else {
        const result = await response.json()
    ...
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="text-center">
          ...
          <p className="text-gray-500">Loading your Hindi files...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="flex items-center mb-4 md:mb-6">
        <div className="bg-orange-100 p-1.5 md:p-2 rounded-full mr-2 md:mr-3">
          <FileText className="h-5 w-5 md:h-6 md:w-6 text-orange-600" />
        </div>
        <h1 className="text-2xl md:text-3xl font-bold text-brand-navy">Hindi Files</h1>
      </div>

      <p className="text-sm md:text-base text-gray-600 mb-6 md:mb-8">
        Upload and manage your Hindi files, notes, and resources.
      </p>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="files">
          <Card>
            <CardHeader className="p-4 md:p-6">
              <CardTitle className="text-lg md:text-xl">Hindi Files</CardTitle>
              <CardDescription className="text-sm">Manage your uploaded Hindi files</CardDescription>
            </CardHeader>
              ...
              {files.length > 0 ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-500 mb-6">
                    You haven't uploaded any Hindi files yet.
                  </p>
        ...
        <TabsContent value="upload">
          <Card>
            <CardHeader className="p-4 md:p-6">
              <CardTitle className="text-lg md:text-xl">Upload Hindi File</CardTitle>
              <CardDescription className="text-sm">
                Upload PDF files for Hindi subject
              </CardDescription>
            </CardHeader>
            <CardContent className="p-4 md:p-6 pt-0 md:pt-0">
