...
export default function PdfAssessmentPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost/pdf-assessments/${pdfId}`, {
          headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Submit answers
      const response = await fetch("http://localhost/assessment", {
        method: "POST",
        headers: {
      ...
      localStorage.setItem(`assessment_result_${assessmentId}`, JSON.stringify(result))
